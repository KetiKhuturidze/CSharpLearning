...
namespace Linq
{
    ...
    public static class AggregationOperations
    {
        ...
        public static IEnumerable<(string customerId, int orderCount)> CustomersOrdersCount()
        {
            ...
            return customers.Select(x => (x.CustomerId, x.Orders.Length));
        }
        ...
        public static IEnumerable<(string category, int productCount)> ProductsInCategoryCount()
        {
            ...
            var categoryCounts = products.GroupBy(product => product.Category).Select(group => (group.Key, group.Count()));
        ...
        public static int SumByLength()
        {
            ...
            return words.Select(x => x.Length).Sum();
        }
        ...
        public static IEnumerable<(string category, int totalUnitsInStock)> TotalUnitsInStock()
        {
            ...
            return products.GroupBy(x => x.Category).Select(x => (x.Key, x.Sum(prod => prod.UnitsInStock)));
        }
        ...
        public static IEnumerable<(string category, decimal cheapestPrice)> GetCheapestPrice()
        {
            ...
            return products.GroupBy(x => x.Category).Select(x => (x.Key, x.Min(y => y.UnitPrice)));
        }
        ...
        public static IEnumerable<(string category, decimal mostExpensivePrice)> GetMostExpensivePrice()
        {
            ...
            return products.GroupBy(x => x.Category).Select(x => (x.Key, x.Max(y => y.UnitPrice)));
        }
        ...
        public static IEnumerable<(string Category, decimal averagePrice)> AveragePrice()
        {
            ...
            return products.GroupBy(x => x.Category).Select(x => (x.Key, x.Average(y => y.UnitPrice)));
        }
        ...
        public static double Aggregate()
        {
            ...
            return doubles.Aggregate((x, next) => x * next);
        }
        ...
        public static double SeededAggregate()
        {
            ...
            double finalBalance = attemptedWithdrawals
                .Aggregate(100, (balance, withdrawal) => Math.Max(balance - withdrawal, 20));
