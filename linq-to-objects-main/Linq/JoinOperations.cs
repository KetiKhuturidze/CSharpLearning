...
namespace Linq
{
    ...
    public static class JoinOperations
    {
        ...
        public static IEnumerable<(string category, string productName)> JoinQuery()
        {
            ...
            var answ = from cat in categories
                       join pr in products on cat equals pr.Category
                       select (cat, pr.ProductName);
        ...
        public static IEnumerable<(string category, IEnumerable<Product> productsName)> GroupJoinQuery()
        {
            ...
            var answ = from cat in categories
                       join pr in products on cat equals pr.Category into prodNames
                       select (cat, prodNames);
        ...
        .GroupJoin(
            ...
            (category, productGroup) => new { Category = category, Products = productGroup })
        .SelectMany(
            x => x.Products.DefaultIfEmpty(new Product { ProductName = "(No products)" }),
            (category, product) => (category.Category, product.ProductName));
