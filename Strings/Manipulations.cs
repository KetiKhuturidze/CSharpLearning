...
namespace Strings
{
    public static class Manipulations
    {
        ...
        {
            string template = ", !";
            template = template.Insert(0, greeting.Trim());
        ...
        {
            template = template.Replace("{name}", name.ToUpperInvariant(), StringComparison.InvariantCulture);
            return template.Replace("{greeting}", greeting.ToLowerInvariant(), StringComparison.InvariantCulture);
        }
